...
Thereafter, we will import the [Passenger Volume by Origin Destination Bus Stops]{.underline} data set for **August** and the sub-zone boundary of URA Master Plan 2019.
...
After populating the community flow, we will combine it into **busstop** sf data frame.

-   `st_intersection()` is used to perform point and polygon overly

-   `dplyr::select()` is used to retain selected columns

Thereafter, we will save a copy of the output in rds format and reload it into the environment.

After running the code, we noticed that 5 bus stops have been excluded as it is beyond the boundary of Singapore.

#### 2.3.2.2 Combine busstop_mpsz into commuting flows

Next, we will append the planning subzone code from busstop_mpsz data frame into various commuting flows data frame as seen in section 2.3.1. To ensure that all bus stops are distinct, we added `unique()` function into our code.
...
#### 2.3.2.3 Combine commuting flows into mpsz

With the incorporated result, we will do a `left_join()` to append the results from the previous section into the **mpsz** sf data frame. As there might be point without **BUS_STOP_N**, and **TRIPS**, `dplyr::mutate()` is used to force a zero.
